(function ($) {
 ...
 function getHeaderHeight() {
  ...
  if ($(".left-modern-header .left-modern-sidebar").length) {
   headerHeight =
    headerHeight + $(".left-modern-header .left-modern-sidebar").outerHeight();
  }
  return headerHeight;
 }
 ...
 function menuPosition(element) {
  ...
  if (
   ...
   var menuRightPosition =
    windowWidth - (simpleDropdown.offset().left + simpleDropdown.outerWidth());
 ...
 if (typeof imagesLoaded === "function") {
  $(".portfolio-wrapper, .shop-wrapper").each(function () {
   ...
   _this.imagesLoaded(function () {
    ...
    if (typeof $.fn.isotope === "function") {
     var currentPortfolioActive = $(".portfolio-filter > li.active > a").attr(
      "data-filter"
     );
 ...
 const waveTextAnimation = (target, options) => {
  ...
  waveText
   ...
   .add({
    ...
    translateY:
     content.length > 1 ? (direction === "down" ? [0, 20] : [0, -20]) : [0, 0],
       ...
       parentSectionObj.imagesLoaded(function () {
        if (overlapGap.length > 0) {
         var gapSectionHeight =
          overlapGap.outerHeight() + (Math.abs(overlayTop) - sectionPaddingTop);
